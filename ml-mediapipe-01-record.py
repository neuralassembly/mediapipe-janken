...
# 手の骨格保存用の関数（自分の手の画像で学習したい方のみ用いる）
def save_hand(mode, results, debug_image):
    global gu_file_count
    global choki_file_count
    global pa_file_count

    # 21 joints, 2 coordinates and visibility
    data_array = np.empty((21, 3), float)
    ...
    for index, landmark in enumerate(results.multi_hand_landmarks[0].landmark):
        ...
        data_array[index, 0] = landmark_x
        data_array[index, 1] = landmark_y
        data_array[index, 2] = landmark.visibility

    if mode == 'g':
        filename = 'data_gu{0:03d}.csv'.format(gu_file_count)
        gu_file_count += 1
    elif mode == 'c':
        filename = 'data_choki{0:03d}.csv'.format(choki_file_count)
        choki_file_count += 1
    elif mode == 'p':
        filename = 'data_pa{0:03d}.csv'.format(pa_file_count)
        pa_file_count += 1
...
def main():
    ...
    while True:
        ...
        if key == 27 or key == ord('q'):
            ...
            save_hand('g', results, debug_image)
        elif key == ord('c'):
            save_hand('c', results, debug_image)
        elif key == ord('p'):
            save_hand('p', results, debug_image)
